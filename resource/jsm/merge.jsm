...
var EXPORTED_SYMBOLS = ["mergeObjects"];

// Simple merge, not deep merge
// objectB's properties has higher priority
function mergeObjects(objectA, objectB) {
  var resultObject = {};
  for (let attrname in objectA) {
    resultObject[attrname] = objectA[attrname];
  }
  for (let attrname in objectB) {
    resultObject[attrname] = objectB[attrname];
  }
  return resultObject;
}
